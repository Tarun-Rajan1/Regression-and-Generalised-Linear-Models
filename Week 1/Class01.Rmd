...
```{r}
#Code
install.packages(c("texreg","stargazer"),repos = "http://cran.us.r-project.org")
...
```{r}
#code
ads <- read.csv("/Users/tarunrajan/Library/Mobile Documents/com~apple~CloudDocs/ST300/Week 1/Advertising.csv")
head(ads)
ads <- ads[, c(-1)]
str(ads)
#ads <- cbind(ads$TV, ads$radio, ads$newspaper, ads$sales)
#ads <- ads[, c(2, 3, 4, 5)]
```
...
nrow(model.matrix(lm_sales_TV))
ncol(model.matrix(lm_sales_TV))
...
lm_sales_TV <- lm(sales~TV, data = ads)
lm_sales_radio <- lm(sales~radio, data = ads)
lm_sales_newspaper <- lm(sales~newspaper, data = ads)
